...
Prefix prompts are prompts used for most modern transformers, where the model predicts the words after this prompt. In earlier models, such as BERT, models could predict words (e.g. <MASK>) in the middle of the prompt. Your job is to be able to tell whether a paper is related to (or simply contains) hard prefix prompting or prompt engineering. Please note that a paper might not spell out that it is using "hard prefix" prompting and so it might just say prompting. In this case, you should still rate it as relevant to the topic of prompt engineering. Please also note, that a paper that focuses on training a model as opposed to post-training prompting techniques is considered irrelevant. Provide a response in JSON format with two fields: 'rating' (a string that is one of the following categories: 'highly likely relevant', 'somewhat likely relevant', 'neutrally likely relevant', 'somewhat likely not relevant', 'highly likely not relevant') indicating relevance to the topic of prompt engineering) and 'reasoning' (that justifies your reasoning)"""

    user_message = f"Title: '{title}', Abstract: '{abstract}'. Rate its relevance to the topic of prompt engineering as one of the following categories: 'highly likely relevant', 'somewhat likely relevant', 'neutrally likely relevant', 'somewhat likely not relevant', 'highly likely not relevant',  and provide text from the abstract that justifies your reasoning"
